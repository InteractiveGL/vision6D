...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.ec0 = self.encoder_layer(3, 32, batchnorm=False)
    ...
    @staticmethod
    def encoder_layer(in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False, batchnorm=True, activation: Optional[nn.Module] = None):
    ...
    @staticmethod
    def decoder_layer(in_channels, out_channels, kernel_size=2, stride=1, padding=0, output_padding=0, dilation=1, bias=True, activation: Optional[nn.Module] = None):
    ...
    def forward(self, x):
        ...
        del d1_u, d1_v, d_u, d_v, e7, syn0, syn1, syn2

        return d_uv

# Inherit from the provided U-Net class
class ConfidenceUNet(UNet):  
    def __init__(self):
        super(ConfidenceUNet, self).__init__()  # Initialize the parent class
        
        # Add a branch for confidence map prediction at the end
        self.confidence_branch = nn.Sequential(
            nn.Conv2d(64, 64, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv2d(64, 1, kernel_size=3, padding=1),
            nn.Sigmoid()  # Sigmoid activation to squash values between 0 and 1
        )

    def forward(self, x):
        d_uv = super(ConfidenceUNet, self).forward(x)  # Primary prediction from parent U-Net
        confidence_map = self.confidence_branch(d_uv)  # Confidence map prediction
        return d_uv, confidence_map